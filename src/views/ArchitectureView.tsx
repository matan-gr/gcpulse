...
import { Layers, ArrowRight, Star, Zap, Box, Grid, Server, Database, Cloud, Shield, Cpu, Globe, ExternalLink, Filter, PenTool } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <PageHeader
        ...
        description="Explore reference architectures, design patterns, and best practices for building scalable and secure applications on Google Cloud."
        badge="Design Patterns"
        icon={PenTool}
        gradient="from-indigo-600 to-violet-600"
        stats={[
          { label: 'Patterns', value: items.length },
          { label: 'Categories', value: categories.length - 1 }, // Subtract 'All'
          { label: 'Featured', value: 'Weekly' }
        ]}
      />

      {/* Filters */}
      <div className="flex items-center space-x-2 overflow-x-auto pb-2 md:pb-0 custom-scrollbar max-w-full">
        <div className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-500 mr-2 flex-shrink-0">
          <Filter size={18} />
        </div>
        {categories.map(cat => (
          <button
